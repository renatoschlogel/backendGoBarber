...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (isBefore(hourStart, new Date())) {
      return res.status(400).json({ error: 'Past dates are nor permited!' });
    }

    const checkAvaisability = await Appointment.findOne({
      where: { provider_id, canceled_at: null, date: hourStart },
    });

    if (checkAvaisability) {
      return res
        .status(400)
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error: 'You can only cancel appointments 2 houres in advance!',
      });
    }
